...
def train_model(model, criterion, optimizer, num_epochs=30):
    ...
    for epoch in range(num_epochs):
        ...
        for j, data in enumerate(train_dataset):
            ...
            H = outputs[0].size()[2]
            W = outputs[0].size()[3]
            interp_8x = nn.Upsample(size=(int(H), int(W)), mode='bilinear', align_corners=True)    
            labels_8x = interp_8x(labels)
            
            H = outputs[2].size()[2]
            W = outputs[2].size()[3]
            interp_16x = nn.Upsample(size=(int(H), int(W)), mode='bilinear', align_corners=True)    
            labels_16x = interp_16x(labels)
            ...
            H = inputs[2].size()[2]
            W = inputs[2].size()[3]
            interp_resize = nn.Upsample(size=(int(H), int(W)), mode='bilinear', align_corners=True)    
