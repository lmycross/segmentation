...
def main(train_args):
    ...
    train_dataset = DataLoader(
            ...
            batch_size=train_args.batch_size, shuffle=True, num_workers=4)
    valid_dataset = DataLoader(
            Loaddata(path, split="val", transform=transform, joint_transform=val_joint_transform),
            batch_size=train_args.batch_size, shuffle=False, num_workers=4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', nargs='?', type=float, default=1e-3, 
                        help='Learning Rate')
    parser.add_argument('--weight_decay', nargs='?', type=float, default=2e-4, 
                        help='Weight decay')
